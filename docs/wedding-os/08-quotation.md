...
**Status: DESIGN — nothing here is implemented.** Written 20 Sep 2026 against `main` @ `ab416c3`.
...
### 6.3 Accept / reject (decision needed — see §11 Q1)
...
stage, so this mirrors how the sale really happens. The data model leaves room for a tokenized public accept link later
(no schema change needed beyond an optional token).
...
bypassed. Friction to decide (§11 Q2): the map forces `QUOTATION_SENT → NEGOTIATION → WON`, so an *accepted* quote
cannot reach `WON` directly.
...
3. Otherwise create one `Invoice` in `DRAFT`: client name/phone/city from the booking or source, `eventDate`/`eventType`
   ...
   `gstAmount = 0`** (tax on an advance is a CA question — §11 Q4), `notes` referencing the quote and its total, linked
...
| 5 | (Optional, §11 Q2) allow `QUOTATION_SENT → WON` when an accepted quote exists | Pipeline friction. |
...
## 11. Open questions (need an answer before / during build)

| # | Question | Recommendation |
|---|---|---|
| Q1 | **Who records acceptance in V1?** Staff on behalf of the customer, or also a customer-facing accept link? | Staff-recorded now (matches how deals close over WhatsApp/phone); ship the tokenized link as a fast-follow. |
| Q2 | Allow `QUOTATION_SENT → WON` when an `ACCEPTED` quote exists? | Yes — one guarded rule instead of forcing a meaningless `NEGOTIATION` step. Needs sign-off since it changes the CRM state machine. |
| Q3 | Advance entered as fixed ₹ or %? | Store the ₹ amount; the editor may accept a % and convert it. |
| Q4 | Tax on the automatic advance invoice | Keep `gstEnabled = false` until the CA confirms treatment of advances; needs the CA's answer, not an engineering one. |
| Q5 | Quotes from Leads? | Allowed by the API, not in the V1 UI. |
...
| S2 | Send / revise / accept / reject / expiry, activity logging, status chips, prepared WhatsApp message, pipeline hint | M |
