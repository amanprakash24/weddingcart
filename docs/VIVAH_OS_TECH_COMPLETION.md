...
| P0-5 | **No way to record a non-Razorpay payment** (cash / UPI / bank transfer). Payments exist only via a Razorpay payment-link webhook, and only for an invoice that already belongs to a Wedding. | Booking → Payment |
...
### Decisions needed from the founding team (block the exact shape of P0-1/P0-5)
1. **Golden-path shape.** In code, `Lead`, `Consultation` and `Enquiry` are three *parallel* capture tables merged in the CRM inbox — there is no Lead → Consultation link. Keep it that way (and start the Golden test at one of them), or introduce a real Lead → Consultation → Enquiry chain?
2. **Consultation → Enquiry duplicates.** PR #101 deliberately allows several Enquiries per Consultation (one per vendor). The handoff says "prevent duplicate conversion". Which rule wins?
3. **Payment sequencing.** Handoff order is Quote → Booking → **Payment → Wedding**. Code order is Booking → **Wedding → Invoice → Payment**, because an invoice needs a `weddingId` before a payment link can exist. Change the code, or change the flow?
...
|---|---|---|---|---|---|---|
| Lead → Consultation | **PARTIAL** (design mismatch) | `Lead`, `Consultation`, `Enquiry` are independent tables; unified only in the CRM inbox (`services/leadInbox.service.ts`). `model Lead` has no Consultation link. | No conversion exists between them. No duplicate detection on phone/email in `lead.service` / `enquiry.service` / `consultation.service`. | P1 | Decision #1. Add phone-based duplicate warning in CRM. | CRM test: same phone submitted twice → surfaced/blocked |
| Consultation → Enquiry | **PARTIAL** | PR #101: `Enquiry.consultationId` (nullable, indexed, not unique); admin "Start Enquiry" prefills from the consultation (`components/AdminClient.tsx`); `enquiry.service.create` connects the relation. Delete guards: `services/enquiry.service.ts`, `consultation.service.ts` (PR #95). | Multiple enquiries per consultation allowed by design; handoff wants duplicate conversion blocked. Vendor is not carried (Consultation has none) — picker required. | P1 | Decision #2, then enforce or document. | `services/enquiry.service.create.test.ts` exists; add duplicate-conversion case per decision |
...
| Booking → Payment | **BLOCKED** (design) | `Booking` has `total` only — no advance/balance/payments. Payments hang off `Invoice` → `Wedding` (`services/payment.service.ts`; webhook returns `handled:false` when `invoice.weddingId` is null). | Handoff pays before the Wedding exists; code cannot. No Booking → Invoice link. | **P0** | Decision #3. | Golden step 7 |
...
|---|---|---|
| 20–26 Sep | Answer decisions #1–#3. Quotation (P0-1) incl. approval + booking link and duplicate-booking guard. Real OTP test (P0-7). Merge/verify PR #102. Stand up the resettable staging seed + first DB-backed test (P0-6). | Consultation → Enquiry → Quotation → Booking passes on staging |
